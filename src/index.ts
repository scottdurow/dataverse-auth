#!/usr/bin/env node
import { app, BrowserWindow } from "electron";
import { AuthenticationContext, TokenResponse } from "adal-node";

console.log(process.argv);
const tenant = process.argv[2]; //"develop1vs.onmicrosoft.com";
const envUrl = process.argv[3]; //"develop1v9demo.crm11.dynamics.com";
console.log(`Authenticating for Tenant:'${tenant}' and Environment:'${envUrl}`);

// eslint-disable-next-line @typescript-eslint/no-use-before-define
main();

function adalAuth(redirectUrl: string): Promise<unknown> {
  const authorityHostUrl = "https://login.windows.net/common";
  const server = envUrl;
  const regex = /(?<=code=)[^&]*/gm;
  const codeMatch = regex.exec(redirectUrl);
  if (!codeMatch) throw new Error("Cannot find code in redirect");

  // Extract code from redirectUrl

  console.log("adalAuth");
  // eslint-disable-next-line @typescript-eslint/no-this-alias
  return new Promise((resolve, reject) => {
    const context = new AuthenticationContext(authorityHostUrl);
    context.acquireTokenWithAuthorizationCode(
      codeMatch[0],
      "app://58145B91-0C36-4500-8554-080854F2AC97",
      "https://" + server,
      "51f81489-12ee-4a9e-aaae-a2591f45987d",
      "",
      (err, tokenResponse) => {
        if (err) {
          console.log("Error");
          console.log(err);
          reject(err);
        } else {
          console.log("Success");
          console.log(tokenResponse);
          resolve(tokenResponse);
        }
      },
    );
  });
}

function authenticate(): Promise<unknown> {
  return new Promise((resolve, reject) => {
    let loginComplete = false;
    // Create the browser window.
    const win = new BrowserWindow({
      width: 800,
      height: 600,
      alwaysOnTop: true,
      autoHideMenuBar: true,
      titleBarStyle: "hidden",
      webPreferences: {
        nodeIntegration: true,
      },
    });
    const url = `https://login.microsoftonline.com/${tenant}/oauth2/v2.0/authorize?client_id=51f81489-12ee-4a9e-aaae-a2591f45987d&response_type=code&haschrome=1&redirect_uri=app%3A%2F%2F58145B91-0C36-4500-8554-080854F2AC97&scope=openid`;
    // Navigate to the get code page
    win.loadURL(url);

    win.on("closed", function() {
      if (!loginComplete) {
        reject("Login Closed");
      }
    });

    win.webContents.on("will-redirect", function(event, newUrl) {
      // Check if this is the success callback
      if (newUrl.startsWith("app://58145b91-0c36-4500-8554-080854f2ac97")) {
        // Stop the redirect to the app: endpoint
        event.preventDefault();
        loginComplete = true;

        adalAuth(newUrl).then(
          function(tokenResponse) {
            const token = tokenResponse as TokenResponse;
            console.log("Token Aquisition Completed");
            win.close();
            resolve(token);
          },
          function(err) {
            console.log("Token Aquisition failed:" + JSON.stringify(err));
            reject(err);
          },
        );
      }
    });
  });
}

function main(): void {
  console.log(process.argv);

  app
    .whenReady()
    .then(authenticate)
    .then(
      function() {
        console.log("COMPLETED");
      },
      function(reason) {
        console.log("ERROR:" + reason);
      },
    );
}
